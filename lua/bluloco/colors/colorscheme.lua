local hsl = require("bluloco.lib.hsl.type")
local shade = require("bluloco.lib.shade")
...
colors["Normal"] = { fg = t.fg, bg = t.bg } -- normal text
...
colors["DiagnosticVirtualTextError"] = { DiagnosticError, bg = t.bg.mix(t.error, 20) } -- Used for "Error" diagnostic virtual text.
colors["DiagnosticVirtualTextWarn"] = { DiagnosticWarn, bg = t.bg.mix(t.warning, 20) } -- Used for "Warn" diagnostic virtual text.
colors["DiagnosticVirtualTextInfo"] = { DiagnosticInfo, bg = t.bg.mix(t.info, 20) }    -- Used for "Info" diagnostic virtual text.
colors["DiagnosticVirtualTextHint"] = { DiagnosticHint, bg = t.bg.mix(t.hint, 20) }    -- Used for "Hint" diagnostic virtual text.
...
-- SagaWinbarObject = {}
-- SagaWinbarUnit = {}
colors["SagaWinbarFile"] = { fg = t.terminalCyan }
...
for group, attrs in pairs(colors) do
  for key, value in pairs(attrs) do
    if key == "gui" and config.gui == nil then
      attrs[key] = nil
    elseif key == "blend" then
      -- do nothing
    elseif key == "fg" or key == "bg" or key == "sp" then
      if type(value) == "table" and value.hex ~= nil then
